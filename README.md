...
- 动态订阅：`http://127.0.0.1:819/sub`
- 全量静态文件：`http://127.0.0.1:819/all.yaml`
- 旧静态目录仍可继续提供 `output` 目录下的文件，例如 `http://127.0.0.1:819/sub/all.yaml`

### 动态订阅参数

动态接口为 `GET /sub`，支持通过查询参数筛选数据库中的最新检测结果。
...
- `http://127.0.0.1:819/sub`：返回默认媒体阶段可用节点
- `http://127.0.0.1:819/sub?test=0`：返回仅通过测活的节点
- `http://127.0.0.1:819/sub?test=1&speed=100`：返回测速通过且速度大于等于 `100KB/s` 的节点
- `http://127.0.0.1:819/sub?test=2&speed=500`：返回媒体阶段通过且速度大于等于 `500KB/s` 的节点

### 本地数据库字段说明

程序会在输出目录下维护本地轻量数据库 `subs.db`，用于支撑动态订阅查询。
...
- 每次完整检测结束后，原有 `batch=0` 数据会整体降级为 `batch=-1`
- 本轮新结果会写入 `batch=0`
- 动态订阅查询时优先读取 `batch=0`
- 如果 `batch=0` 没有匹配结果，则自动回退到 `batch=-1`
